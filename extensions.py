...
# Redis client — module-level singleton, same pattern as db/mail above.
# Unlike db/mail, it does NOT need an .init_app(app) call:
# redis.Redis.from_url(...) doesn't open a connection at construction
# time (connections are opened lazily from an internal pool on first
# command), and it reads REDIS_URL directly from the environment rather
# than from Flask's app.config.
#
...
def _create_redis_client():
    ...
    try:
        # from_url() infers the connection class (TLS or not) from the
        # URL scheme (rediss:// vs redis://). Passing an explicit `ssl=`
        # kwarg alongside from_url() is invalid — it gets forwarded
        # straight into the connection class's __init__, and the plain
        # (non-TLS) Connection class has no `ssl` parameter, which raises:
